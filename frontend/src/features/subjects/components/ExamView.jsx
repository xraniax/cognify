...
import { AlertTriangle, Brain, CheckCircle2, Clock3, FileText, Flag, RefreshCw, Save, Trophy, FileDown, Sparkles } from 'lucide-react';
...
const ExamView = ({ examData: rawExamData, examId: propExamId, subjectId, isExpanded = false, isLiveGenerating = false }) => {
    ...
    const isAdaptive = exam?.mode === 'adaptive' || examData?.mode === 'adaptive';
    const targetTotal = isAdaptive ? (exam?.targetTotal || examData?.targetTotal || 15) : (exam?.questions?.length || 0);
    ...
    useEffect(() => {
        ...
        MaterialService.getAttempt(examId).then((res) => {
            ...
            if (attempt.batchIndex !== undefined) setBatchIndex(attempt.batchIndex);
    ...
    // Keep exam data in sync during live-generating (streaming) — do NOT overwrite
    // adaptive-extended state where extra batches have already been appended.
    useEffect(() => {
        if (!examData?.questions?.length) return;
        setExam(prev => {
            if (!prev) return { ...examData };
            // If the in-memory exam already has more questions than the prop (adaptive batches
            // were appended via handleNextBatch), keep the richer state to avoid regression.
            if ((prev.questions?.length || 0) > (examData.questions?.length || 0)) return prev;
    ...
    const submitExam = useCallback(async () => {
        ...
        try {
            ...
            if (subjectId && examResult && !analyticsSubmittedRef.current) {
    ...
    const handleNextBatch = useCallback(async () => {
        if (isLoadingNextBatch || isSubmitting) return;
        setIsLoadingNextBatch(true);
        setError('');
        try {
            const batchSize = exam.batchSize || 5;
            const currentBatchQuestions = exam.questions.slice(total - batchSize, total);
            const unanswered = currentBatchQuestions.filter(q => {
                const qAnswer = answers[q.id] || { selectedAnswers: [], answerText: '', blankAnswers: [], matchAnswers: {} };
                return ['short_answer', 'problem', 'scenario'].includes(q.type)
            ...
            if (unanswered.length > 0) {
                toast.error(`Please answer all questions in the current batch before adapting.`);
                setIsLoadingNextBatch(false);
                return;
            }
            ...
            const res = await MaterialService.nextBatch(payload);
            const resultData = res?.data?.data;
            if (resultData) {
                if (resultData.completed) {
                    await submitExam();
                } else if (Array.isArray(resultData.questions) && resultData.questions.length > 0) {
                    const initialAnswers = {};
                    ...
                    setExam(prev => {
                        const updatedQuestions = [...prev.questions, ...resultData.questions];
                        return {
                            ...prev,
                            questions: updatedQuestions,
                        };
                    });
                    setBatchIndex(resultData.batchIndex);
                    setCurrentIndex(total);
                    toast.success("AI has successfully adapted questions based on your performance!");
                } else {
                    await submitExam();
                }
            }
        } catch (err) {
            setError(err.message || 'Failed to adapt and load the next batch of questions.');
            toast.error('Adaptation failed. Please try again.');
        } finally {
            setIsLoadingNextBatch(false);
        }
    }, [answers, exam, examId, isLoadingNextBatch, isSubmitting, serializeAnswers, submitExam, total]);
    ...
    const progressPct = targetTotal > 0 ? Math.round((answeredCount / targetTotal) * 100) : 0;
    ...
    if (!exam || !Array.isArray(exam.questions) || exam.questions.length === 0 || !currentQuestion) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-gray-500">
                {isLiveGenerating ? (
                    <>
                        <div className="w-20 h-20 rounded-[2rem] bg-indigo-50 flex items-center justify-center mb-8 animate-pulse border-4 border-white shadow-xl">
                            <Brain className="w-10 h-10 text-indigo-500" />
                        </div>
                        <h3 className="text-2xl font-black text-indigo-950 mb-3 tracking-tight">Architecting Mock Exam</h3>
                        <p className="text-sm font-medium text-gray-400 max-w-sm text-center leading-relaxed">
                            Cognify is synthesizing multiple question formats based on your selected materials. This may take a moment.
                        </p>
                    </>
                ) : (
                    <>
                    ...
                    <div className="relative rounded-[3rem] border-8 border-white bg-white shadow-2xl p-8 overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                    <div className="w-10 h-10 rounded-2xl bg-indigo-50 flex items-center justify-center border-2 border-white shadow-sm">
                                        {isAdaptive ? <Brain className="w-5 h-5 text-indigo-600" /> : <FileText className="w-5 h-5 text-indigo-600" />}
                                    </div>
                                    <span className="text-indigo-400 text-[10px] font-black uppercase tracking-[0.3em]">
                                        {isAdaptive ? 'Adaptive Arena' : 'Knowledge Arena'}
                                    </span>
                                    {isAdaptive && (
                                        <span className="flex items-center gap-1 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full shadow-sm">
                                ...
                                <div className="flex items-center gap-4 mt-3">
                                    <div className="flex items-center gap-2 text-gray-400 text-[10px] font-black uppercase tracking-widest">
                                        ...
                                        {answeredCount}/{targetTotal} Complete
                                    </div>
                                    {isAdaptive && (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                {/* In adaptive mode, hide the top-level Final Submit until the full exam is built */}
                                {(!isAdaptive || total >= targetTotal) && (
                                    <button
                                        onClick={submitExam}
                                        disabled={isSubmitting || !!result || isLoadingNextBatch}
                ...
                <div id="exam-content" className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-8">
                    <div className="space-y-6">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {/* Adaptive batch-loading overlay */}
                                {isLoadingNextBatch && (
                                    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/95 backdrop-blur-sm rounded-[2.5rem]">
                                        <div className="w-20 h-20 rounded-[2rem] bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mb-6 shadow-2xl shadow-indigo-200 animate-pulse">
                                            <Brain className="w-10 h-10 text-white" />
                                        </div>
                                        <h3 className="text-xl font-black text-indigo-950 tracking-tight mb-2">AI is Adapting</h3>
                                        <p className="text-sm font-medium text-gray-400 max-w-xs text-center leading-relaxed">
                                            Analysing your performance and calibrating the next set of questions to your mastery level...
                                        </p>
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex gap-4">
                                <button
                                    ...
                                    disabled={currentIndex === 0 || isLoadingNextBatch}
                                ...
                                <button
                                    onClick={() => {
                                        if (currentIndex >= total - 1) {
                                            // Adaptive: load next batch if exam not yet complete
                                            if (isAdaptive && total < targetTotal) {
                                                handleNextBatch();
                                            } else {
                                                submitExam();
                                            }
                                    ...
                                    disabled={isSubmitting || isLoadingNextBatch}
                                    className="px-8 py-4 rounded-[1.5rem] bg-indigo-600 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-indigo-100 hover:scale-105 active:scale-95 transition-all disabled:opacity-30 flex items-center gap-2"
                                >
                                    {isLoadingNextBatch ? (
                                        <>
                                        ...
                                        isAdaptive && total < targetTotal
                                            ? <><Sparkles className="w-4 h-4" />Adapt & Continue</>
                            ...
                            <div className="flex gap-3">
                                {isAdaptive && (
                                    <div className="flex items-center gap-2 px-4 py-2 bg-indigo-50 rounded-2xl border-2 border-indigo-100">
                                        <Brain className="w-3.5 h-3.5 text-indigo-500" />
                                        <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">
                                            {total}/{targetTotal} Questions
                                        </span>
                                    </div>
                                )}
                                <button
                        ...
                        <div key={`print-${q.id}`} className="break-inside-avoid border-b border-gray-100 pb-10">
                            <div className="flex gap-6 mb-4">
                                ...
                                <div className="pt-2 flex-1">
                                    ...
                                    {/* Blank Paper Mode: Non-choice questions */}
                                    {!result && ['short_answer', 'problem', 'scenario'].includes(q.type) && (
                                        <div className="ml-2 mt-4 h-32 w-full border-2 border-dashed border-gray-100 rounded-2xl" />
                                    )}

                                    {!result && q.type === 'fill_blank' && (
                                        <div className="ml-2 mt-4 space-y-4">
                                            {Array.from({ length: q.blankAnswers?.length || 1 }).map((_, i) => (
                                                <div key={i} className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex-1 border-b-2 border-gray-100 h-10" />
                                                </div>
                                            ))}
                                        </div>
                                    )}

                                    {!result && q.type === 'matching' && (
                                        <div className="ml-2 mt-4 space-y-3">
                                            {(q.pairs || []).map((pair, pIdx) => (
                                                <div key={pIdx} className="flex items-center gap-4">
                                                    <div className="p-4 border-2 border-gray-100 rounded-xl bg-gray-50/50 min-w-[200px] font-bold text-gray-700">{pair.left}</div>
                                                    <div className="flex-1 border-b-2 border-dotted border-gray-200" />
                                                    <div className="w-48 h-12 border-2 border-dashed border-gray-100 rounded-xl" />
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {result && detail && !['single_choice', 'multiple_choice'].includes(q.type) && (
